...
def parse_single_product(product_soup: BeautifulSoup) -> Product:
    return Product(
        ...
        description=(
            product_soup.select_one(".description")
            .text.replace("\xa0", " ")
        ),
        price=float(product_soup.select_one(".price").text.replace("$", "")),
        rating=len(product_soup.select(".ws-icon.ws-icon-star")),
        num_of_reviews=(
            int(product_soup.select_one("p.pull-right").text.split()[0])
        )
    )
...
def more_button(url: str) -> BeautifulSoup:
    ...
    try:
        more = driver.find_element(By.CLASS_NAME, "ecommerce-items-scroll-more")
    except NoSuchElementException:
        more = None
...
def write_to_csv(category: str, products: list[Product]) -> None:
    with open(
            f"{category}.csv", "w", newline="", encoding="utf-8"
    ) as csv_file:
