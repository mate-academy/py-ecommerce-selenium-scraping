...
def get_single_product(driver: WebElement) -> Product:
    return Product(
        ...
        num_of_reviews=int(driver.find_element(
            By.CSS_SELECTOR, ".ratings .pull-right").text.split()[0]),
    )


def parse_single_page_product(url: str, driver: WebDriver) -> [Product]:
    driver.get(url)
    ...
    try:
        ...
        pass
    try:
        more_button = driver.find_element(By.CLASS_NAME,
    ...
    with webdriver.Chrome() as new_driver:
        for page in LIST_URLS:
            write_products_to_csv(
                parse_single_page_product(page[0], new_driver),
                page[1])
