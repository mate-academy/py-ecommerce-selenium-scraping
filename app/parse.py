...
def click_more_button():
    while True:
        try:
            more_button = webdriver.find_element(By.CLASS_NAME, "ecomerce-items-scroll-more")
        except:
            break
        else:
...
def create_product(product_card):
    return Product(
                ...
                price=float(product_card.find_element(By.CLASS_NAME, "price").text.replace("$", "")),
                rating=get_rating(product_card),
                num_of_reviews=int(product_card.find_element(By.CLASS_NAME, "review-count").text.split(" ")[0])
            )
...
def write_page_content(file_path, products):
    data = [[product.title, product.description, product.price, product.rating, product.num_of_reviews] for product in products]
