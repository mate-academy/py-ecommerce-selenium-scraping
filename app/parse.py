...
def get_single_product(product: Tag):
    return Product(
        title=product.select_one(".title")["title"],
        description=product.select_one(".description").text,
        price=float(product.select_one(".price").text.replace("$", "")),
        rating=len(product.select(".ws-icon-star")),
        num_of_reviews=int(product.select_one("p.pull-right").text.split()[0])
    )


def get_products_from_the_page(
        page_url: str,
        driver: WebDriver
) -> list[Product]:
    driver.get(page_url)
...
def get_all_products() -> None:
    with webdriver.Chrome() as driver:
        for name, url in URLS.items():
            products = get_products_from_the_page(url, driver)
