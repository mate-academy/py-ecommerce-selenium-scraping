...
def accept_cookie(page: Page):
    page.query_selector(".acceptCookies").click()


def btn_more_click_until_exists(page: Page):
    page.wait_for_timeout(100)
    btn_more = page.query_selector(".ecomerce-items-scroll-more")
    if not btn_more:
        return

    is_visible = "none" not in btn_more.evaluate("element => element.style.display")
...
def get_category_name(page: Page):
...
def convert_info_to_object(product_info: ElementHandle) -> Product:
    ...
    price = product_info.query_selector(".price").text_content().replace("$", "")

    rating = len(product_info.query_selector_all(".ratings .glyphicon-star"))
    n_reviews = int(product_info.query_selector(".ratings .pull-right").text_content().split()[0])
    description = product_info.query_selector(".description").text_content()
...
def go_next_category_page(page: Page):
    ...
    if current_category:
        try:
            next_category = (
                current_category.evaluate_handle("element => element.nextElementSibling")
                .query_selector("a")
            )
            ...
            try:
                next_category = (
                    current_category.evaluate_handle("element => element.parentElement")
                ...
                n = page.query_selector("li.active + li")
                if n:
                    n.click()
                    return
                is_stop = True
...
def save_products_to_csv(products: list[Product], category_name: str):
